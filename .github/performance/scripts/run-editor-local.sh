#!/usr/bin/env bash
# Local Core vs Blockera block-editor (client) performance compare.
#
# Usage:
#   npm run test:performance:editor:local
#   npm run test:performance:editor:local -- --skip-build --skip-env-start --base-url=http://localhost:8888
#
set -euo pipefail
...
usage() {
	cat <<'EOF'
Usage: npm run test:performance:editor:local -- [options]

Runs Blockera vs Core block-editor client benchmarks (selecting blocks,
switching workspace tabs), then writes editor-report.md.

Samples are fixed at 10 (+1 throwaway) per Gutenberg's post-editor pattern.
...
Example:
  npm run test:performance:editor:local -- --skip-build --skip-env-start

Example (force performance wp-env):
...
echo "=== Blockera local editor performance benchmark ==="
...
# Prefer the performance wp-env (TT5 + BLOCKERA_PERF_BENCHMARK). Server-Timing
# is not required for editor metrics, but confirms the perf config is loaded.
perf_env_ready() {
	local header
...
start_or_restart_env() {
	...
	echo "Restarting wp-env so performance.json mappings apply..."
...
if [[ -n "${FORCE_ENV_RESTART}" ]]; then
	...
	if curl -sf -o /dev/null "${WP_BASE_URL}"; then
		...
		if ! perf_env_ready; then
			start_or_restart_env "Perf config not detected on running env (stale wp-env without performance.json mappings)."
		else
			echo "Perf environment already present (reusing env)."
		fi
	else
		start_perf_env
	fi
...
if ! perf_env_ready; then
	echo "Warning: Server-Timing header missing (env may not be using performance.json)."
	echo "Editor benchmarks can still run; for a clean env use:"
	echo "  npm run test:performance:editor:local -- --skip-build --force-env-restart"
fi
...
bash .github/performance/scripts/run-editor-benchmarks.sh

echo "Comparing editor results..."
set +e
node tests/performance/compare-editor-results.js
COMPARE_CODE=$?
set -e

REPORT="${PERF_RESULTS_DIR}/editor-report.md"
if [[ -f "$REPORT" ]]; then
	echo ""
	echo "=== Report: ${REPORT} ==="
	cat "$REPORT"
