 ...
 */
function blockera_test_avatar_resolve_email( $id_or_email ) {
 ...
 * @return array
 */
function blockera_test_avatar_filter_data( $args, $id_or_email ) {
	$email = strtolower( trim( blockera_test_avatar_resolve_email( $id_or_email ) ) );
	if ( '' === $email || 'hello@blockera.ai' !== $email ) {
		return $args;
	}

	$rel  = blockera_test_avatar_fixture_rel_path();
...
add_filter( 'get_avatar_data', 'blockera_test_avatar_filter_data', 10, 2 );
