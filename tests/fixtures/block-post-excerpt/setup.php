<?php
/**
 * Setup for the post excerpt block.
 * 
...
if (!is_array($data) || !isset($data['post']) || !isset($data['excerpt'])) {
	throw new \Exception('Invalid data file format: ' . $data_file);
}

$post_id = $this->factory()->post->create([
	'post_title'   => $data['post']['post_title'] ?? 'Test Post for Post Excerpt',
	'post_content' => $post_content,
	'post_status'  => $data['post']['post_status'] ?? 'publish',
	'post_type'    => $data['post']['post_type'] ?? 'post',
]);

// Try direct database update to bypass WordPress filters for sanitization of excerpt
global $wpdb;

// Update post_excerpt directly in database
