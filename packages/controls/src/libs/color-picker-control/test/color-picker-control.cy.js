...
describe('Color-Picker Control', () => {
	...
	context('Functional Tests', () => {
		...
		it('picks correct color by dragging the color board', () => {
			cy.withDataProvider({
				component: (
					<ColorPickerControl isOpen={true} isPopover={false} />
				),
			...
			cy.get('.react-colorful__saturation-pointer').trigger('mousedown', {
				which: 1,
				pageX: 600,
				pageY: 100,
			});
			cy.get('.react-colorful__saturation-pointer').trigger('mousemove', {
				which: 1,
				pageX: 600,
				pageY: 600,
			});
			cy.get('.react-colorful__saturation-pointer').trigger('mouseup');

			// visual and data assertion
			cy.get('.react-colorful__saturation-pointer')
				.find('.react-colorful__pointer-fill')
				.should('have.css', 'backgroundColor', 'rgb(96, 109, 154)')
				.then(() => {
					expect(getControlValue(name)).to.be.equal('#616e9c');
				});
		});
