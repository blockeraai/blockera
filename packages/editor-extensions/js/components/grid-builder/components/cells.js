...
export const Cells = ({
	...
	const activeArea = blockeraGridAreas.find(
		(item) => item.id === activeAreaId
	);

	const targetArea = blockeraGridAreas.find(
				...
				case 'bottomRight':
					{
						if (
							activeArea['row-end'] ===
								newMergedArea['row-end'] &&
							activeArea['column-end'] ===
								newMergedArea['column-end']
						)
							return;
					}
					break;
	...
	return blockeraGridAreas?.map((item) => {
