...
if ( ! function_exists( 'blockera_resolve_layout_fallback_block_gap' ) ) {
	/**
	 * Resolve layout fallback blockGap matching core {@see wp_render_layout_support_flag()}.
	 *
	 * Priority: active style variation → block global styles → root global styles → block-type default.
	 * Uses request-cached merged styles so render_block does not cold-start core resolvers.
	 *
	 * @param string       $block_name           Block name (e.g. core/group).
	 * @param string|array $default_fallback_gap Block-type default from block.json.
	 * @param array        $block_attrs          Block attributes (needs className for variations).
	 * @return string|array|null
	 */
	function blockera_resolve_layout_fallback_block_gap( $block_name, $default_fallback_gap, array $block_attrs ) {
		static $global_styles   = null;
		static $per_block_cache = array();
		static $variation_cache = array();

		$block_class_name = $block_attrs['className'] ?? '';
		$has_variation    = $block_class_name && str_contains( $block_class_name, 'is-style-' );

		if ( ! $has_variation && isset( $per_block_cache[ $block_name ] ) ) {
			return $per_block_cache[ $block_name ];
		}

		if ( $has_variation ) {
			$variation_cache_key = $block_name . "\0" . $block_class_name;
			if ( isset( $variation_cache[ $variation_cache_key ] ) ) {
				return $variation_cache[ $variation_cache_key ];
			}
		}

		$fallback_gap_value = $default_fallback_gap;

		if ( null === $global_styles ) {
			$global_styles = JSONResolver::get_merged_styles();
		}

		$variation_block_gap_value = null;

		// Only hit the styles registry when a variation class may be active (core guard).
		if ( $has_variation && $block_name ) {
		...
		$global_block_gap_value = $variation_block_gap_value
			?? $global_styles['blocks'][ $block_name ]['spacing']['blockGap']
		...
		if ( ! $has_variation ) {
			$per_block_cache[ $block_name ] = $fallback_gap_value;
		} else {
			$variation_cache[ $variation_cache_key ] = $fallback_gap_value;
		}
