...
if (! function_exists('blockera_get_breakpoints')) {
	 ...
	 * @return array
	 */
	function blockera_get_breakpoints(): array {

		$user_id              = get_current_user_id();
		$fallback_breakpoints = blockera_core_config('breakpoints.list');
		...
		$blockera_editor_settings = get_user_meta($user_id, 'blockera_editor_settings', true);
		$breakpoints              = $blockera_editor_settings['breakpoints'] ?? $fallback_breakpoints;

		if (! is_plugin_active('blockera-pro/blockera-pro.php')) {
			
			$breakpoints = blockera_update_breakpoints($breakpoints);
		}
...
if (! function_exists('blockera_update_breakpoints')) {
     ...
     * @return array
     */
    function blockera_update_breakpoints( array $breakpoints): array {
		...
		if (empty($blockera_editor_settings)) {
			$blockera_editor_settings = [
				'breakpoints' => $breakpoints,
			];
		} else {
			$blockera_editor_settings['breakpoints'] = $breakpoints;
		}

		update_user_meta($user_id, 'blockera_editor_settings', $blockera_editor_settings);
