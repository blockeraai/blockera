...
import { useState, useRef, useEffect, useMemo, memo, forwardRef } from '@wordpress/element';
...
function areTabRenderPropsEqual(
	...
	return (
		...
		prevProps.isIconOnlyPinnedTabsEnabled === nextProps.isIconOnlyPinnedTabsEnabled &&
 ...
 */
const Tab = memo(forwardRef<HTMLDivElement, TabComponentProps>(function Tab({
	tab,
	isActive,
	hasUnsavedChanges,
	isPinned = false,
	...
	return (
		<>
			<div
				...
				className={`blockera-tabs-tab ${isActive ? 'is-active' : ''} ${isPinned ? 'is-pinned' : ''
					} ${isRenamed ? 'renamed-tab' : ''} ${isLocked ? 'is-locked' : ''} ${(isTabIconsEnabled || isIconOnly) ? 'show-icons' : ''} ${isIconOnly ? 'is-icon-only' : ''} ${canDrag ? 'is-draggable' : ''} ${isDragging ? 'is-dragging' : ''}`}
				role="tab"
				tabIndex={-1}
			...
			>
				<Button
					...
					variant="tertiary"
					onPointerDown={canDrag && dragListeners?.onPointerDown ? (e) => {
						// Call the drag listener to start drag when clicking button
						if (dragListeners.onPointerDown) {
							(dragListeners.onPointerDown as any)(e);
						}
					} : undefined}
					tabIndex={0}
				>
					...
					{isLocked && (
						<Tooltip
							text={sprintf(
								...
								__('Locked by %s', 'blockera-tabs'),
								lockUser?.name || __('another user', 'blockera-tabs')
							)}
						>
					...
					{(isTabIconsEnabled || isIconOnly) && !isLocked && statusIcon && (
					...
					{/* Show when icons enabled OR when in icon-only mode for pinned tabs */}
					{(isTabIconsEnabled || isIconOnly) && !isLocked && !statusIcon && typeIcon && (
					...
					{!isIconOnly && (
						<span className="blockera-tabs-tab-title">{displayTitle}</span>
					)}
