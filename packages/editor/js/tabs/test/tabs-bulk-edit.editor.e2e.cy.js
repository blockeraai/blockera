...
describe('Workspace tabs: Bulk edit', () => {
	...
	it('should open two tabs when using posts list bulk action Edit All in Editor', () => {
		...
		cy.get(unpinnedTabRoots).contains(titleA, { matchCase: false }).click();
		cy.tabsGetActiveTitle().should('contain.text', titleA);

		cy.get(unpinnedTabRoots).contains(titleB, { matchCase: false }).click();
