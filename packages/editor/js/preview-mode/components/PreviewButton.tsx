 ...
 * @returns The button component wrapped in Fill.
 */
export default function PreviewButton({
	...
	const shortcutsDisplay = sprintf(
		...
		__('%1$s or %2$s %3$s', 'blockera-tabs'),
		toggleShortcut,
		newTabShortcut,
		__('(new tab)', 'blockera-tabs')
	);
	...
	return (
		<Fill name="blockera/slots/editor-header-settings">
			<div className="blockera-preview-button-wrapper">
				<Tooltip
					text={__('Live frontend preview', 'blockera-tabs')}
					shortcut={shortcutsDisplay}
				>
					<a
						href={href}
						className={`components-button is-compact ${isActive ? 'is-pressed' : ''}`}
						onClick={(event) => {
							if (disabled) {
						...
						aria-label={__('Live frontend preview', 'blockera-tabs')}
