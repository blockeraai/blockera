/**
 * Width → WP Compatibility (Global Styles)
 * Playwright e2e test
 */
const {
	openSiteEditor,
...
test.describe('Width → WP Compatibility (Global Styles)', () => {
	// Mu-plugin paths configuration: map test titles to their mu-plugin paths
	// Mu-plugin paths are relative to plugin root
	const muPluginPaths = {
		'Simple Value':
			'packages/editor/js/extensions/libs/size/test/global-styles/fixtures/width-setup-1.php',
	};
	...
	const before = async (page) => {
		...
		await page.locator('button[id="/blocks/core%2Fcolumn"]').click();
	};
	...
	test.describe('Column Block', () => {
		test.describe('Simple Value', () => {
			test('Simple Value', async ({ page }) => {
				...
				const root = globalStylesRecord?.['core/column'];

				// WP data should come to Blockera
				const blockeraWidth1 = root?.blockeraWidth?.value;

				expect('50%').toEqual(blockeraWidth1);
				...
				// set width
				await setSizeControlValue(page, 'Width', '75%');
				...
				const root2 = globalStylesRecord2?.['core/column'];
				const dimensionsWidth = root2?.dimensions?.width;

				expect('75%').toEqual(dimensionsWidth);
				...
				// clear width
				await clearSizeControlValue(page, 'Width');
				...
				const root3 = globalStylesRecord3?.['core/column'];
				const dimensionsWidth3 = root3?.dimensions?.width;
				const blockeraWidth3 = root3?.blockeraWidth?.value;

				expect(undefined).toEqual(dimensionsWidth3);
				expect('').toEqual(blockeraWidth3);
			});
		});
	});
});
