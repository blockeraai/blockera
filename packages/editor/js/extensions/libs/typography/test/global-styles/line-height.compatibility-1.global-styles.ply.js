	...
	test.describe('Paragraph Block', () => {
		test.describe('Simple Value', () => {
			test('Simple Value', async ({ page }) => {
				...
				const blockeraLineHeight1 = root?.blockeraLineHeight?.value;

				expect('1.8').toEqual(blockeraLineHeight1);
				...
				const typographyLineHeight = root2?.typography?.lineHeight;

				expect('2.5').toEqual(typographyLineHeight);
				...
				const typographyLineHeight3 = root3?.typography?.lineHeight;
