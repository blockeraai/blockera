<?php
/**
 * Temporary mu-plugin to modify theme.json for font color tests (simple value)
 * This filter adds simple font color value for core/paragraph block using add_filter
 * Sets font color via color.text (WordPress theme.json schema for global styles)
 */
add_filter('blockera_theme_json_data_theme', function($theme_json) {
	$data = $theme_json;
	
	// Ensure styles.blocks structure exists
	if (!isset($data['styles']['blocks'])) {
		$data['styles']['blocks'] = [];
	}
	
	// Ensure core/paragraph block exists
	if (!isset($data['styles']['blocks']['core/paragraph'])) {
		$data['styles']['blocks']['core/paragraph'] = [];
	}
	
	// Ensure styles.blocks.core/paragraph.color structure exists
	if (!isset($data['styles']['blocks']['core/paragraph']['color'])) {
		$data['styles']['blocks']['core/paragraph']['color'] = [];
	}
	
	// Add font color configuration based on WordPress valid schema
	// Using color.text for simple color values (global styles context)
	$data['styles']['blocks']['core/paragraph']['color']['text'] = '#ffbaba';
	
	// Update the theme.json data
	return $data;
}, 10, PHP_INT_MAX);
