...
describe('String Manipulation and Search Utilities', () => {
	 ...
	 *
	 * This suite tests the `search` function, which searches a list of items by
	...
	describe('search', () => {
		...
		it('should return all items when search input is empty', () => {
			expect(search(items, categories, collections, '')).toEqual(items);
		});

		it('should filter items by normalized search terms', () => {
			expect(search(items, categories, collections, 'media')).toEqual(
				items
			);
			expect(search(items, categories, collections, 'core')).toEqual([
				items[0],
			]);
		});
	});
