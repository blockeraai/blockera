...
test.describe('Background Repeat → WP Compatibility (Global Styles)', () => {
	...
	test.describe('Group Block', () => {
		test.describe('Background Repeat', () => {
			test('No Repeat Value', async ({ page }) => {
				...
				// change repeat to repeat (default, should be removed)
				const popover = page.locator('.components-popover').last();
				await popover.locator('button[data-value="repeat"]').click();
				...
				const backgroundRepeat3 = root3?.background?.backgroundRepeat;
