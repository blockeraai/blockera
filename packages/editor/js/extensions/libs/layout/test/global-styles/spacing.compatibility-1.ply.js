...
test.describe('Spacing → WP Compatibility (Global Styles)', () => {
	...
	test.describe('Paragraph Block', () => {
		test.describe('Simple Value', () => {
			test('Simple Value', async ({ page }) => {
				...
				const blockeraSpacing1 = root?.blockeraSpacing?.value;
				...
				const spacingPaddingTop = root2?.spacing?.padding?.top;

				expect('25px').toEqual(spacingPaddingTop);
