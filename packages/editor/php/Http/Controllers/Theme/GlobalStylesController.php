...
class GlobalStylesController extends \WP_REST_Posts_Controller {
	...
	protected function get_post( $id ) {
		$error = new \WP_Error(
			...
			__( 'No global styles config exist with that id.', 'blockera' ),
			array( 'status' => 404 )
		);
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && $post && ! $this->check_update_permission( $post ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to edit this global style.', 'blockera' ),
		...
		if ( ! $this->check_read_permission( $post ) ) {
			return new \WP_Error(
				'rest_cannot_view',
				__( 'Sorry, you are not allowed to view this global style.', 'blockera' ),
				array( 'status' => rest_authorization_required_code() )
			);
		}
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				'id'       => array(
					'description' => __( 'ID of global styles config.', 'blockera' ),
					'type'        => 'string',
					'context'     => array( 'embed', 'view', 'edit' ),
					'readonly'    => true,
				),
				'styles'   => array(
					'description' => __( 'Global styles.', 'blockera' ),
					'type'        => array( 'object' ),
					'context'     => array( 'view', 'edit' ),
				),
				'settings' => array(
					'description' => __( 'Global settings.', 'blockera' ),
					'type'        => array( 'object' ),
					'context'     => array( 'view', 'edit' ),
				),
				'title'    => array(
					'description' => __( 'Title of the global styles variation.', 'blockera' ),
					'type'        => array( 'object', 'string' ),
					'default'     => '',
					'context'     => array( 'embed', 'view', 'edit' ),
					'properties'  => array(
						'raw'      => array(
							'description' => __( 'Title for the global styles variation, as it exists in the database.', 'blockera' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit', 'embed' ),
						),
						'rendered' => array(
							'description' => __( 'HTML title for the post, transformed for display.', 'blockera' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit', 'embed' ),
							'readonly'    => true,
						),
	...
	public function get_theme_item_permissions_check( $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		return new \WP_Error(
			...
			__( 'Sorry, you are not allowed to access the global styles on this site.', 'blockera' ),
			array(
	...
	protected function validate_custom_css( $css ) {
		if ( preg_match( '#</?\w+#', $css ) ) {
			return new \WP_Error(
				...
				__( 'Markup is not allowed in CSS.', 'blockera' ),
				array( 'status' => 400 )
			);
		}
		return true;
