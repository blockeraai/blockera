...
const SPLIT_LOWER_UPPER_RE: RegExp = /([\p{Ll}\d])(\p{Lu})/gu;
const SPLIT_UPPER_UPPER_RE: RegExp = /(\p{Lu})([\p{Lu}][\p{Ll}])/gu;
// Used to iterate over the initial split result and separate numbers.
const SPLIT_SEPARATE_NUMBER_RE: RegExp = /(\d)\p{Ll}|(\p{L})\d/u;
// Regexp involved with stripping non-word characters from the result.
const DEFAULT_STRIP_REGEXP: RegExp = /[^\p{L}\d]+/giu;
// The replacement value for splits.
const SPLIT_REPLACE_VALUE: string = '$1\0$2';
// The default characters to keep after transforming case.
const DEFAULT_PREFIX_SUFFIX_CHARACTERS: string = '';
 ...
 */
export function pascalSnakeCase(
	input: string,
	options?: SplitPrefixSuffixOptions
): string {
...
function lowerFactory(locale: boolean | string): (string) => string {
	return locale === false || 'string' !== typeof locale
		? (input) => input.toLowerCase()
		: (input) => input.toLocaleLowerCase(locale);
}

function upperFactory(locale: boolean | string): (string) => string {
	return locale === false || 'string' !== typeof locale
